...
public class BuildSanitizer : IPreprocessBuildWithReport
{
    ...
    public void OnPreprocessBuild(BuildReport report)
    {
        ...
        case BuildTarget.iOS: CheckBuildSettingsiOS(); break;
    ...
    /// <summary>
    /// Ensures correct project settings for iOS
    /// - Minimum API Level = 17.0
    /// </summary>
    public static void CheckBuildSettingsiOS()
    {
        if(Int16.Parse(PlayerSettings.iOS.targetOSVersionString.Split('.')[0]) < 17)
            Debug.LogError(productName + ": Minimum iOS SDK version needs to be upper than 17.0");
        
#if !ARFOUNDATION_PRESENT
        Debug.LogError(productName + ": AR Foundation should be installed since in iOS Track is only supported in fusion mode");
#endif
    }
    ...
    {
        // Min SDK version 28 / Android 9.0
        if (PlayerSettings.Android.minSdkVersion < AndroidSdkVersions.AndroidApiLevel28)
            Debug.LogError(productName + ": Minimum Android SDK version needs to be " + AndroidSdkVersions.AndroidApiLevel28.ToString());
