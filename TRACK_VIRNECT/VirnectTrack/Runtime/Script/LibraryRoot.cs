...
namespace VIRNECT
{
    ...
    public class LibraryRoot
    {
...
#elif UNITY_IOS
        const string dll = "__Internal"; // Internal script

        /// <summary>
        /// Set camera buffer (YUV Channel) reference into Track
        /// </summary>
        /// <param name="data">Buffer reference</param>
        ...
        /// <returns>Success or failure of setting camera buffer</returns>
        [DllImport(dll, EntryPoint = "setCameraYUVBuffers")]
        unsafe protected static extern bool InternalSetCameraFramebuffer(void* data, int width, int height, int format);

        /// <summary>
        /// Set camera calibration into Track
        /// </summary>
        /// <param name="intrinsics">Camera intrinsic parameter</param>
        /// <returns>Success or failure of setting camera calibration</returns>
        [DllImport(dll, EntryPoint = "setCameraCalibration")]
        protected static extern bool InternalSetCameraCalibration(float[] intrinsics);
#elif UNITY_ANDROID
    /// <summary>
    ...
    /// <summary>
    /// VIRNECT TRACK Hardware class
    /// </summary>
    protected static AndroidJavaObject trackHWClass = null;

    /// <summary>
    /// New camera frame is available or not
    /// </summary>
    protected static bool isAvailableTexture = false;

    /// <summary>
    /// VIRNECT TRACK Hardware interface proxy
    /// </summary>
    class TrackHardwareInterface : AndroidJavaProxy
    {
        public TrackHardwareInterface(): base("com.virnect.hw.TrackHardwareInterface") {}
        public void onNewFrameAvailable() { isAvailableTexture = true; }
        public void onNewAccFrameAvailable(float[] accSensorData, long timestamp) {}
        public void onNewGyroFrameAvailable(float[] gyroSensorData, long timestamp) {}
        public void onNewDepthFrameAvailable(int width, int height, long timestamp) {}
    }
    ...
    protected static bool InitializeAndroidPlugins()
    {
        ...
        if (trackClass == null)
        {
            Debug.LogError("TRACK - Failed to get Android Interface Library. Please check interface library is in plugins/Android folder");
            return false;
        }
    ...
    /// <summary>
    /// Initialize Android hardware plug-ins
    /// </summary>
    /// <returns>Success or failure of getting context and class</returns>
    protected static bool InitializeAndroidHardwarePlugins()
    {
        ...
        trackHWClass = new AndroidJavaObject("com.virnect.hw.TrackHardware", activityContext, true, false, false);
        trackHWClass.Call("setHardwareCallback", new TrackHardwareInterface());
        if (trackHWClass == null)
        {
            Debug.LogError("TRACK - Failed to get Android Hardware Library. Please check hardware library is in plugins/Android folder");
            return false;
        }
    ...
    protected static AndroidJavaObject getTargetInformationArray(string[] targets)
    {
        if (targets.Length > targetInformationArraySize)
        {
            targetInformationArray = javaArrayClass.CallStatic<AndroidJavaObject>("newInstance", new AndroidJavaClass("com/virnect/Common$TargetInformation"), targets.Length);
            targetInformationArraySize = targets.Length;

            // Populate with initial content
            for (int i = 0; i < targets.Length; i++)
                javaArrayClass.CallStatic("set", targetInformationArray, i, new AndroidJavaObject("com/virnect/Common$TargetInformation", targets[i]));
        }
    ...
    const string dll = "";
        ...
        /// <param name="savePath">Path for saving map target</param>
